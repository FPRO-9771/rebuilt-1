"""
Tests for AssistAimSelector -- neutral-zone corner aim selection (Assist mode).
...
def test_outside_zone_returns_hub():
    ...
    assert out == hub
    assert sel.in_assist_mode is False
...
def test_inside_zone_returns_nearest_blue_corner_top():
    ...
    assert sel.in_assist_mode is True
...
def test_not_teleop_always_returns_hub_and_clears_latch():
    ...
    # First latch Assist mode in teleop.
...
def test_hysteresis_holds_assist_mode_inside_buffer():
    """After latching Assist mode, crossing the boundary by less than the
