...
def test_zero_velocity_zero_corrections():
    ...
    tracking, lead = compute_movement_correction(0.0, 0.0, 3.0, TEST_CON_SHOOTER)
    assert tracking == 0.0
    assert lead == 0.0
...
def test_lateral_velocity_produces_tracking_correction():
    ...
    tracking, _ = compute_movement_correction(0.0, 2.0, 3.0, TEST_CON_SHOOTER)
...
def test_tracking_direction_matches_velocity():
    ...
    pos_tracking, _ = compute_movement_correction(0.0, 1.0, 3.0, TEST_CON_SHOOTER)
    neg_tracking, _ = compute_movement_correction(0.0, -1.0, 3.0, TEST_CON_SHOOTER)
...
def test_lead_correction_with_velocity_lead_enabled():
    ...
    _, lead = compute_movement_correction(0.0, 2.0, 3.0, config)
    assert lead != 0.0
...
def test_lead_correction_zero_at_short_distance():
    ...
    config = {**TEST_CON_SHOOTER, "velocity_lead_enabled": True}
    _, lead = compute_movement_correction(0.0, 2.0, 0.3, config)
    assert lead == 0.0


def test_forward_velocity_does_not_affect_tracking():
    """Forward velocity (vx) does not affect tracking correction."""
    tracking_with_vx, _ = compute_movement_correction(3.0, 0.0, 3.0, TEST_CON_SHOOTER)
    tracking_without, _ = compute_movement_correction(0.0, 0.0, 3.0, TEST_CON_SHOOTER)
