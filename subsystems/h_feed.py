"""
Horizontal feed subsystem.
Powered by a KrakenX60 motor for moving Fuel horizontally.
"""
...
from constants import MOTOR_IDS, CON_H_FEED


class HFeed(SubsystemBase):
    """Horizontal feed for transporting Fuel. Uses voltage control."""

    def __init__(self):
        super().__init__()
        self.motor = create_motor(MOTOR_IDS["h_feed"])
    ...
    class _ManualCommand(Command):
        """Joystick control of horizontal feed."""

        def __init__(self, feed: "HFeed", speed_supplier: Callable[[], float]):
            super().__init__()
            self.feed = feed
        ...
        def execute(self):
            ...
            voltage = speed * CON_H_FEED["max_voltage"]
    ...
    class _RunAtVoltageCommand(Command):
        """Run horizontal feed at fixed voltage."""

        def __init__(self, feed: "HFeed", voltage: float):
            super().__init__()
            self.feed = feed
